...
test('format trims Unicode separators', (t) => {
  t.is(
    format({
      ...
      lastName: '\xA0Unbreakable\xA0',
    }),
    'Arnold\x20Unbreakable'
  );
});
...
test('format handles title token', (t) => {
  ...
  t.is(
    format({ lastName: 'doe' }, { tokens: '[title] [firstName] [lastName]' }),
    'Doe'
  );
});
...
test('format handles France country specific rules', (t) => {
  ...
  t.is(format({ lastName: 'du bellay' }, { country: 'FR' }), 'Du Bellay');

  t.is(
    format({ firstName: 'GÉRARD', lastName: "d'aboville" }, { country: 'FR' }),
    "Gérard d'Aboville"
  );
  t.is(format({ lastName: "d'aboville" }, { country: 'FR' }), "d'Aboville");

  t.is(
    format({ firstName: 'guy ', lastName: 'des cars' }, { country: 'FR' }),
    'Guy des Cars'
  );
  t.is(format({ lastName: 'DES CARS' }, { country: 'FR' }), 'des Cars'); // TODO should be "Des Cars"
});
